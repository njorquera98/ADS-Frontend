...
export class TablaSolicitudesComponent {
  ...
  getAsignatura(solicitud: Solicitud): Asignatura | undefined {
    const ayudantia = this.ayudantiasService.ayudantias.find(
      (val) => val.id_ayudantia === solicitud.id_ayudantia
    );
  ...
  getUser(solicitud: Solicitud): Estudiante | undefined {
    return this.estudiantesService.estudiantes.find(
      (val) => val.id_usuario === solicitud.id_usuario
    );
  }

  eliminarSolicitud(solicitud: Solicitud) {
    const index = this.solicitudesService.solicitudes.findIndex(
      (val) => val.id_solicitud === solicitud.id_solicitud
    )!;
  ...
  aprobarSolicitud(solicitud: Solicitud) {
    ...
    const id_edit = this.solicitudesService.solicitudes.findIndex(
      (val) => val.id_solicitud === solicitud.id_solicitud
    );
