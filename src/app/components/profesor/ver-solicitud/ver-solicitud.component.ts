...
export class VerSolicitudComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.solicitud = this.solicitudesService.solicitudes.find(
      (val) =>
        val.id_solicitud ===
        +this.activatedRoute.snapshot.params['id_solicitud']
    );
    console.log(this.solicitud?.fecha.toString());
  }

  getAyudantiaString(solicitud: Solicitud) {
    const ayudantia = this.ayudantiasService.ayudantias.find(
      (v) => v.id_ayudantia === solicitud.id_ayudantia
    );
    const asignatura = this.asignaturasService.asignaturas.find(
      (v) => v.id_asignatura === ayudantia?.id_asignatura
    );
    return `${asignatura?.nombre} ${asignatura?.letra}`;
  }

  getNombreEstudiante(solicitud: Solicitud) {
    const estudiante = this.estudiantesService.estudiantes.find(
      (v) => v.id_usuario === solicitud.id_usuario
    );
